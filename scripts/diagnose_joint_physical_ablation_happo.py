#!/usr/bin/env python3
"""Joint HAPPO diagnostics with fewer physical agents than the checkpoint schema.

This script is intentionally separate from diagnose_joint_happo.py. It keeps
the checkpoint observation schema fixed, but lets diagnostics instantiate fewer
physical UAVs/UGVs to test robustness to missing agents.
"""
...
def _scenario_kwargs(checkpoint_dir: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    # Preserve the checkpoint schema before applying physical-agent ablations.
    # In this script, --n-drones/--n-ugvs change only the instantiated VMAS
    # agents; obs_schema_* stays at checkpoint width so actor inputs match.
    ...
    if getattr(args, "obs_schema_n_drones", None) is not None:
        scenario_kwargs["obs_schema_n_drones"] = int(args.obs_schema_n_drones)
    if getattr(args, "obs_schema_n_ugvs", None) is not None:
        scenario_kwargs["obs_schema_n_ground"] = int(args.obs_schema_n_ugvs)
    if getattr(args, "obs_schema_n_survivors", None) is not None:
        scenario_kwargs["obs_schema_n_survivors"] = int(args.obs_schema_n_survivors)
    ...
    *,
    ...
    for step in range(max_steps):
        ...
        if n_active_survivors > 0:
            ...
            scout_auc_sum += 1.0
            confirm_auc_sum += 1.0
        coverage_auc_sum += float(scenario.coverage_grid[0].float().mean().item())
        confidence_auc_sum += float(scenario.uav_confidence_grid[0].float().mean().item())
        auc_steps += 1
    ...
    def survivor_count_hist(ax) -> None:
        ...
        if values:
            ...
            success_counts = Counter(
                ...
                and bool(
                    row.get(
                        "full_confirm_success",
                        row.get("overall_success", False),
                    )
                )
            )
            xs = sorted(counts)
            totals = [counts[x] for x in xs]
            successes = [success_counts.get(x, 0) for x in xs]
            ax.bar(
                xs,
                totals,
                color="#4f7df3",
            ...
            for x, total, successful in zip(xs, totals, successes):
                ax.text(
                    x,
...
def main() -> None:
    ...
    parser.add_argument("--n-drones", "--n-uavs", dest="n_drones", type=int, default=None,
                        help="Physical UAV count for this ablation. Observation schema stays fixed from checkpoint.")
    parser.add_argument("--n-ugvs", "--n-ground", dest="n_ugvs", type=int, default=None,
                        help="Physical UGV count for this ablation. Observation schema stays fixed from checkpoint.")
    parser.add_argument("--n-survivors", type=int, default=None,
                        help="Physical survivor count. Observation survivor slots stay fixed from checkpoint.")
    parser.add_argument("--obs-schema-n-drones", "--schema-n-drones", type=int, default=None,
                        help="Rare override for UAV observation slots. Default preserves checkpoint schema.")
    parser.add_argument("--obs-schema-n-ugvs", "--schema-n-ugvs", type=int, default=None,
                        help="Rare override for UGV observation slots. Default preserves checkpoint schema.")
    parser.add_argument("--obs-schema-n-survivors", "--schema-n-survivors", type=int, default=None,
                        help="Rare override for survivor observation slots. Default preserves checkpoint schema.")
    ...
    if args.n_drones is not None and args.n_drones < 0:
        parser.error("--n-drones must be nonnegative")
    if args.n_ugvs is not None and args.n_ugvs < 0:
        parser.error("--n-ugvs must be nonnegative")
    if args.n_drones == 0 and args.n_ugvs == 0:
        parser.error("at least one physical agent is required")
    if args.n_survivors is not None and args.n_survivors < 1:
        parser.error("--n-survivors must be positive")
    if args.obs_schema_n_drones is not None and args.obs_schema_n_drones < 0:
        parser.error("--obs-schema-n-drones must be nonnegative")
    if args.obs_schema_n_ugvs is not None and args.obs_schema_n_ugvs < 0:
        parser.error("--obs-schema-n-ugvs must be nonnegative")
    if args.obs_schema_n_survivors is not None and args.obs_schema_n_survivors < 1:
        parser.error("--obs-schema-n-survivors must be positive")
    ...
    physical_drones = int(scenario_kwargs.get("n_drones", 0))
    physical_ugvs = int(scenario_kwargs.get("n_ground", 0))
    schema_drones = int(scenario_kwargs.get("obs_schema_n_drones", physical_drones))
    schema_ugvs = int(scenario_kwargs.get("obs_schema_n_ground", physical_ugvs))
    physical_survivors = int(scenario_kwargs.get("n_survivors", 0))
    schema_survivors = int(scenario_kwargs.get("obs_schema_n_survivors", physical_survivors))
    if physical_drones + physical_ugvs <= 0:
        parser.error("resolved scenario has no physical agents")
    if physical_drones > schema_drones:
        parser.error(
            f"physical UAV count ({physical_drones}) exceeds fixed UAV schema ({schema_drones})"
        )
    if physical_ugvs > schema_ugvs:
        parser.error(
            f"physical UGV count ({physical_ugvs}) exceeds fixed UGV schema ({schema_ugvs})"
        )
    if physical_survivors > schema_survivors:
        parser.error(
            f"physical survivor count ({physical_survivors}) exceeds fixed survivor schema ({schema_survivors})"
        )
    try:
    ...
    print(
        ...
        f"schema={schema_drones} UAV slots/{schema_ugvs} UGV slots/"
        f"{schema_survivors} survivor slots, "
    ...
    if actor_file_indices is not None:
        print(f"actor files: {actor_file_indices}")
