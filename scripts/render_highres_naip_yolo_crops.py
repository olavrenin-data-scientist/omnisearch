...
def main() -> None:
    ...
    parser.add_argument("--survivor-rotation-deg", type=float, default=-18.0)
    parser.add_argument(
        "--asset-resample",
        choices=("nearest", "bilinear"),
        default="bilinear",
        help="Resize method for the pasted human asset.",
    )
    parser.add_argument(
        "--human-asset",
        default=None,
    ...
    parser.add_argument(
        "--same-human-asset",
        action="store_true",
        help="Use the same selected human asset for every altitude crop.",
    )
    ...
    shared_human_asset = rng.choice(human_assets) if args.same_human_asset else None
    outputs = []
    ...
    for altitude_m in args.altitude_agl_m:
        ...
        survivor_world = None
        if bbox is not None:
            human_asset_path, human_asset = shared_human_asset or rng.choice(human_assets)
        ...
        outputs.append(
            {
                ...
                "survivor_rotation_deg": float(args.survivor_rotation_deg),
