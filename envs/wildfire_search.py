...
class WildfireSearchScenario(BaseScenario):
    ...
    def make_world(self, batch_dim: int, device: torch.device, **kwargs) -> World:
        ...
        # Disabled by default to keep observation size and existing behavior
        # unchanged. When enabled, decoys are non-survivor landmarks that drones
        # can falsely scout and UGVs can waste trips dismissing.
        self.n_decoys = max(int(kwargs.pop("n_decoys", 0)), 0)
        self.drone_false_positive_rate = min(
            max(float(kwargs.pop("drone_false_positive_rate", 0.0)), 0.0),
            1.0,
        )
