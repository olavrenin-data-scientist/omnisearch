...
class TestThermalModelEdgeCases:
    ...
    def test_thermal_detector_validation(self):
        """SimulationCvAdapter should reject invalid thermal detector backends."""
        from detection.simulation_adapter import SimulationCvAdapter
        with pytest.raises(ValueError, match="thermal_detector"):
            det = object.__new__(SimulationCvAdapter)
            SimulationCvAdapter.__init__(
                det,
                terrain_cache_path="dummy",
                thermal_detector="invalid",
            )


class TestThermalYoloBackend:
    """Tests for the trained-YOLO thermal detection backend."""

    def test_yolo_backend_requires_weights(self, tmp_path):
        """thermal_detector='yolo' without weights must fail clearly."""
        from detection.simulation_adapter import SimulationCvAdapter
        det = object.__new__(SimulationCvAdapter)
        with pytest.raises(FileNotFoundError, match="thermal_yolov8n"):
            SimulationCvAdapter.__init__(
                det,
                terrain_cache_path="dummy",
                thermal_detector="yolo",
                root=tmp_path,  # empty dir: no models/thermal_yolov8n.pt
            )

    def test_yolo_backend_detects_on_rendered_frame(self):
        """End-to-end: rendered TIR frame -> thermal YOLO -> world coordinates."""
        from pathlib import Path as _P
        weights = _P(__file__).resolve().parent.parent / "models" / "thermal_yolov8n.pt"
        if not weights.exists():
            pytest.skip("thermal weights not trained yet")
        ...
        det.grid_size = 100
        det.person_conf = 0.35
        det.person_iou = 0.7
        det.person_device = None
        det.thermal_detector = "yolo"
        det.thermal_model_name = str(weights)
        det._thermal_yolo = None
        det._thermal_seed = 42

        drone = SimpleNamespace(world_xy=(0.0, 0.0))
        altitude_m = 30.0
        footprint_m = 2.0 * altitude_m * _math.tan(_math.radians(65.0) / 2.0)
        footprint_world = footprint_m * det.sim_units_per_meter
        # Survivor 25% of the footprint east of center — clearly in frame.
        survivor = SimpleNamespace(index=0, world_xy=(footprint_world * 0.25, 0.0))

        records = det._run_thermal_yolo_detection(
            drone=drone,
            survivors=[survivor],
            wildfire_state=None,
        ...
        assert detected, "thermal YOLO failed to detect a clear survivor blob"
        hit = next((r for r in detected if r.get("survivor_index") == 0), None)
        assert hit is not None, "detection not matched to the ground-truth survivor"
        est = hit["estimated_world_xy"]
        err = _math.hypot(est[0] - survivor.world_xy[0], est[1] - survivor.world_xy[1])
        assert err < footprint_world * 0.05, f"world-coordinate error too large: {err}"
