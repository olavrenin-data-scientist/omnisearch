...
class HappoCheckpointTests(unittest.TestCase):
    ...
    def test_uav_survivor_diagnostic_build_args(self):
        ...
        self.assertEqual(scenario["r_uav_team_confidence"], 0.0)
    ...
    def test_uav_survivor_diagnostic_uses_current_defaults(self):
        ...
        self.assertEqual(scenario["local_confidence_obs_radius_m"], 60.0)
        ...
        self.assertEqual(scenario["r_uav_inefficient_move"], 0.005)
        self.assertEqual(scenario["uav_inefficient_move_source"], "confidence")
        ...
        self.assertEqual(scenario["uav_start_min_separation_m"], 150.0)
        self.assertEqual(scenario["uav_start_edge_margin_m"], 50.0)
        self.assertEqual(algo_args["model"]["terrain_cnn_single_obs_dim"], 1283)

    def test_uav_survivor_diagnostic_can_set_survivor_count(self):
        _, _algo_args, env_args = build_args(
            num_env_steps=100,
            episode_length=50,
            seed=1,
        ...
        self.assertEqual(scenario["n_drones"], 3)
        self.assertEqual(scenario["n_ground"], 0)
        self.assertEqual(scenario["n_survivors"], 7)
    ...
    def test_joint_diagnostics_can_set_survivor_count(self):
        _, _algo_args, joint_env_args = build_args(
            num_env_steps=100,
            episode_length=50,
            seed=1,
            comms_dropout=0.0,
            entropy_coef=0.01,
            exp_name="joint_diag_survivors",
            joint_survivor_diagnostic=True,
            n_survivors=6,
        )
        _, _algo_args, schema_env_args = build_args(
            num_env_steps=100,
            episode_length=50,
            seed=1,
            comms_dropout=0.0,
            entropy_coef=0.01,
            exp_name="joint_schema_ugv_survivors",
            joint_schema_ugv_diagnostic=True,
            n_survivors=6,
        )

        joint_scenario = joint_env_args["scenario_kwargs"]
        self.assertEqual(joint_scenario["n_survivors"], 6)
    ...
    def test_joint_survivor_diagnostic_accepts_route_cost_greedy_assignment(self):
        _, _algo_args, env_args = build_args(
            num_env_steps=100,
            episode_length=50,
            seed=1,
            comms_dropout=0.0,
            entropy_coef=0.01,
            exp_name="joint_diag_route_cost_greedy",
            joint_survivor_diagnostic=True,
            ugv_target_assignment_mode="route-cost-greedy",
        )
    ...
    def test_joint_schema_ugv_diagnostic_accepts_route_cost_greedy_assignment(self):
        _, _algo_args, env_args = build_args(
            ...
            comms_dropout=0.0,
            entropy_coef=0.01,
            exp_name="joint_schema_ugv_route_cost_greedy",
            joint_schema_ugv_diagnostic=True,
            ugv_target_assignment_mode="route-cost-greedy",
        )
    ...
    def test_can_set_uav_team_confidence_reward(self):
        _, _, uav_env_args = build_args(
            num_env_steps=100,
            episode_length=50,
            seed=1,
    ...
    def test_uav_diagnostic_default_has_no_binary_coverage_observations(self):
        _, algo_args, env_args = build_args(
            num_env_steps=100,
            episode_length=50,
            seed=1,
    ...
    def test_uav_diagnostic_explicit_zero_disables_coverage_observations(self):
        ...
        self.assertFalse(scenario["uav_frontier_obs"])
